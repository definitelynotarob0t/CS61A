class Account:
    ...
    def time_to_retire(self, amount):
        ...
        while updated_balance < amount:
            updated_balance += updated_balance*(self.interest)
            years += 1
        return years
...
def without(s, i):
    ...
    if i == 0:
        if s:
            return s.rest
        else: # indicates that i is greater than length of list
            return s 
    else:
...
"""
duplicate_link(Link(5, Link(4, Link(5))), 5)

s.first == 5:
    s.rest = Link(5, Link(4, Link(5)))
    s.first = 5
    # s = Link(5, Link(5, Link(4, Link(5))))
        return: Link(5, Link(5, duplicate_link(Link(4, Link(5))), 5)
        duplicate_link(Link(4, Link(5))), 5):
            return: Link(4, duplicate_link(Link(5)), 5)
            duplicate_link(Link(5), 5)
                return Link(5, Link(5), duplicate_link(None), val)
                    duplicate_link(None, val)
                return Link(5, Link(5))
            Link(4, Link(5, Link(5)))
        Link(5, LInk(5, Link(4, Link(5, Link(5)))))
