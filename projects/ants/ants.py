...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        if self.exit:                        
            adjacent_place = self.exit
            adjacent_place.entrance = self
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if original_ant.can_contain(new_ant):
                original_ant.ant_contained = new_ant
            
            elif new_ant.can_contain(original_ant):
                new_ant.ant_contained = original_ant
                place.ant = new_ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        curr_place = self.place # begin where ant is
        steps = 0

        # Start at lower_bound (change is only needed if lower_bound isn't 0)
        if hasattr(self, 'lower_bound'):
            if self.lower_bound != 0:
                steps = self.lower_bound
                move_curr_place = self.lower_bound
                while move_curr_place > 0 and curr_place:
        ...
        while True: 
            if curr_place.is_hive: # indicates that the end of the tunnel has been reached
                return None
            
            elif curr_place.bees == []: # if there are no bees in the curr_place, check next place (if within ant's throw range)
                if hasattr(self, 'lower_bound'): 
                    if steps + 1 >= self.lower_bound and steps + 1 <= self.upper_bound:
                        steps += 1
                        curr_place = curr_place.entrance
                    else:
                        return None    
            ...
            else: # bees have been found in the curr_place, return a bee randomly from that place
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bees_in_place = list(self.place.bees) # shallow copy for iteration
       
        # Inflict damage on all bees in place   
        for bee in bees_in_place:     
            bee.reduce_health(amount)
   
        will_die = self.health <= amount
        
        
        # If reducing ant's health would result in it dying, inflict extra damage
        if will_die:
            bees_in_place = list(self.place.bees) # update to reflect changes 
            for bee in bees_in_place:
                bee.reduce_health(self.damage)   
                
         # Reduce health of ant               
...
class HungryAnt(Ant):
    ...
    def eat_bee(self):
        ...
        if bees_in_place:
            bee_to_eat = random_bee(bees_in_place)
    ...
    def action(self, gamestate):
        turns_to_cool = self.cooldown
        if turns_to_cool > 0: # indicates ant is chewing and decremenets turns left for chewing
            self.cooldown = turns_to_cool - 1
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        # If tank ant contains an ant, ensure contained ant also does its action
        if self.ant_contained:
            self.ant_contained.action(gamestate)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # If the insect is not waterproof, reduce the insect's health to 0. 
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        super().action(gamestate) # throw leaf
        ...
        while curr_place:
            if curr_place.ant:
                ants_behind.append(curr_place.ant)
                if curr_place.ant.is_container and curr_place.ant.ant_contained:
                    ants_behind.append(curr_place.ant.ant_contained)
