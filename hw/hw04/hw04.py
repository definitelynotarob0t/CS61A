...
def max_path_sum(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def sum_path(t, curr_max):
        if is_leaf(t):
            return curr_max + label(t)
        else:
            for branch in branches(t):
                return sum_path(branch, (curr_max + label(t))) # only checks the first branch :-/
            
    poss_paths = []
    for branch in branches(t):
        poss_paths.append(sum_path(branch, label(t)))
    
    return max(poss_paths)
